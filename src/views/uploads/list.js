...
import { Row, Col, Spinner, Dropdown, InputGroup, FormControl, Card, Alert, Button } from 'react-bootstrap'
...
const Component = () => {
    ...
    const columns = [
        ...
        { name: 'Nome', acessor: 'extra', column: 'extra', format: 'filename' },
        { name: 'Caminho', acessor: 'path', column: 'path' },
        { name: 'Tamanho', acessor: 'size', column: 'size', format: 'filesize' },
    ...
    const loadData = async (pageLimit, page) => {
        ...
        try {
            let url = `/api/v1/private/upload/list?order=${orderBy}&limit=${pageLimit}&page=${page}&search=${searchField}`
    ...
    const callDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await api.delete(`/api/v1/private/upload/delete/${id}`)
                    Swal.fire({
                                                            ...
                                                            <td key={i}>
                                                                ...
                                                                {r.format ?
                                                                    ...
                                                                    : <>
                                                                        {r.acessor === 'custom'
                                                                            ? <>
                                                                                <Button onClick={() => downloadFile(row.file, row.original_name)} variant="outline-info" className="btn-icon btn-icon-only mb-1">
