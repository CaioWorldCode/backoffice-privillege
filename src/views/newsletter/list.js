...
import { Row, Col, Spinner, Dropdown, InputGroup, FormControl, Card, Alert, Button, Modal } from 'react-bootstrap';
...
const Component = () => {
    ...
    const description = 'Notícias';

    const breadcrumbs = [
        { to: '', text: 'Home' },
        { to: 'newsletter', text: 'Notícias' },
        { to: 'list', text: 'Lista' },
    ]

    const columns = [
        { name: 'Id', acessor: 'id', column: 'id' },
        { name: 'Título', acessor: 'title', column: 'title' },
        { name: 'Descrição', acessor: 'description', column: 'description' },
        { name: 'Ativo', acessor: 'active', column: 'active', format: 'active' },
    ...
    const loadData = async (pageLimit, page) => {
        ...
        try {
            let url = `/api/v1/private/newsletter?order=${orderBy}&limit=${pageLimit}&page=${page}&search=${searchField}`
    ...
    const callEdit = (id) => {
        setRedirectUrl(`/newsletter/edit/${id}`)
        setRedirect(true)
    }
    ...
    const callDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await api.delete(`/api/v1/private/newsletter/${id}`)
                    Swal.fire({
                ...
                <Modal.Body>
                    ...
                    <Row>
                        <Col xs={12} className="mt-4">
                            <span className='text-primary'><b>Título:</b> </span> {dataContent.title}
                        </Col>

                        <Col xs={12} className="mt-4">
                            <span className='text-primary'><b>Descrição:</b> </span> {dataContent.description}
                        </Col>

                        <Col xs={12} className="mt-4">
                            <span className='text-primary'><b>Conteúdo:</b> </span>


                            <div style={{ margin: 5, padding: 5, border: '1px solid #ccc'}}>
                                <div
