...
const Component = () => {
    const title = 'Tipo de notícias';
    const description = 'Tipo de notícias';

    const breadcrumbs = [
        { to: '', text: 'Home' },
        { to: 'config', text: 'Notícias' },
        { to: 'newsletter_type', text: 'Tipo de notícias' },
    ]
    ...
    const loadData = async (pageLimit, page) => {
        ...
        try {
            let url = `/api/v1/private/newsletter_type?order=${orderBy}&limit=${pageLimit}&page=${page}&search=${searchField}`
    ...
    const callLoadEditRegister = async () => {
        ...
        try {
            const response = await api.get(`/api/v1/private/newsletter_type/${idEdit}`)
    ...
    const callDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await api.delete(`/api/v1/private/newsletter_type/${id}`)
                    Swal.fire({
            ...
            <>
                <Row>
                    <Col xs={12} xxl={12} className=" d-flex justify-content-end align-items-end">
                        ...
                        >
                            Novo tipo de notícia <CsLineIcons icon="plus" />
                        </Button>
                    </Col>
                </Row>
                ...
                <Modal.Body>
                    ...
                    <Formik
                        ...
                        onSubmit={async (values, { resetForm }) => {
                            try {
                                const response = await api.post(`/api/v1/private/newsletter_type`, values)
                ...
                <Modal.Body>
                    ...
                    {loadingIdData
                        ...
                        : <Formik
                            ...
                            onSubmit={async (values, { resetForm }) => {
                                try {
                                    const response = await api.post(`/api/v1/private/newsletter_type/${idEdit}`, values)
