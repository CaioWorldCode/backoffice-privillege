...
const Component = () => {
    ...
    const description = 'Contratos';

    const breadcrumbs = [
        { to: '', text: 'Home' },
        { to: 'config', text: 'Configurações' },
        { to: 'contract', text: 'Contratos' },
    ]
    ...
    const loadData = async (pageLimit, page) => {
        ...
        try {
            let url = `/api/v1/private/contract?order=${orderBy}&limit=${pageLimit}&page=${page}&search=${searchField}`
    ...
    const callEdit = (id) => {
        setRedirectUrl(`/contracts/edit/${id}`)
        setRedirect(true)
    }
    ...
    const callDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await api.delete(`/api/v1/private/contract/${id}`)
                    Swal.fire({
