...
const Component = () => {
    const title = 'Tipo de contrato';
    const description = 'Tipo de contrato';

    const breadcrumbs = [
        { to: '', text: 'Home' },
        { to: 'config', text: 'Contratos' },
        { to: 'contract_type', text: 'Tipo de contrato' },
    ]
    ...
    const loadData = async (pageLimit, page) => {
        ...
        try {
            let url = `/api/v1/private/contract_type?order=${orderBy}&limit=${pageLimit}&page=${page}&search=${searchField}`
    ...
    const callLoadEditRegister = async () => {
        ...
        try {
            const response = await api.get(`/api/v1/private/contract_type/${idEdit}`)
    ...
    const callDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await api.delete(`/api/v1/private/contract_type/${id}`)
                    Swal.fire({
                ...
                <Modal.Body>
                    ...
                    <Formik
                        ...
                        onSubmit={async (values, { resetForm }) => {
                            try {
                                const response = await api.post(`/api/v1/private/contract_type`, values)
                ...
                <Modal.Body>
                    ...
                    {loadingIdData
                        ...
                        : <Formik
                            ...
                            onSubmit={async (values, { resetForm }) => {
                                try {
                                    const response = await api.post(`/api/v1/private/contract_type/${idEdit}`, values)
